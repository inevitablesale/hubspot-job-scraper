...
# Hints that a *specific* link is a job detail or "view position" link, not just
# a generic "careers" or "jobs" hub.
JOB_LINK_PATH_HINTS = [
    "/jobs/",
    "/job/",
    "/careers/",
...
class HubspotSpider(scrapy.Spider):
            ...
            if self._is_internal(full_url, host_root) and self._looks_like_career(full_url, text):
                if full_url not in seen:
                    ...
                    yield scrapy.Request(
                        ...
                        meta={"company": company, "job_page": full_url, "root": root},
                    )

    def parse_career_page(self, response):
        """
        Entry point for both on-domain career hubs and off-domain ATS pages.

        1. First, try to extract individual job detail URLs from the page using
           anchors, data-* attributes, and onclick handlers.
        2. If we find concrete job URLs, follow them to `parse_job_page` and
           only score those pages.
        3. As a fallback (for very simple sites), we still allow scoring the
           career page itself.
        """
        company = response.meta["company"]
        base = response.meta.get("root") or response.url

        job_links = self._extract_job_links(response, base)
        ...
        # Fallback: score the page itself if it looks like a concrete role.
        ...
        yield {
            "company": company,
            "job_page": response.meta.get("job_page", response.url),
    ...
    def parse_job_page(self, response):
        """
        Job detail pages (either on the main domain or an ATS host).
        These should contain the actual description, requirements, etc.
        """
        company = response.meta["company"]
        job_page = response.meta.get("job_page", response.url)
    ...
    def _extract_job_links(self, response, base_url):
        """
        Pull out *job detail* links from a careers/ATS hub page.

        Handles:
        - Regular <a href="..."> links with job-ish paths or text
        - Elements with data-href / data-url / data-link / data-target
        - onclick="goToJob('/careers/role-slug/')" patterns
        """
        links = set()

        # 1) Plain anchors with href
        for a in response.css("a[href]"):
            href = a.attrib.get("href")
            if not href:
                continue
            text = " ".join(a.css("::text").getall()).strip().lower()
            candidate = self._safe_urljoin(base_url, href)
            if not candidate:
                continue

            # Heuristic: looks like a job detail link if either path or text
            # carries job-ish hints.
            lower_href = href.lower()
            if any(h in lower_href for h in JOB_LINK_PATH_HINTS) or any(
                t in text for t in JOB_LINK_TEXT_HINTS
            ):
                links.add(candidate)

        # 2) data-* attributes commonly used for job cards
        for attr in ["data-href", "data-url", "data-link", "data-target"]:
            for href in response.xpath(f"//*[@{attr}]/@{attr}").getall():
                candidate = self._safe_urljoin(base_url, href)
                if candidate:
                    links.add(candidate)

        # 3) onclick handlers like onclick="goToJob('/careers/analytics-manager/')"
        onclick_values = response.xpath("//*[@onclick]/@onclick").getall()
        for onclick in onclick_values:
            # Very loose extraction: first quoted path/URL
